...
from VDSRUNK_MF_ra import BATCH_SIZE
from VDSRUNK_MF_ra import PATCH_SIZE
...
def get_test_list(ASCNN_Frames_List1, lowDataList, ASCNN_Frames_List2, labelList):
    doubleTest_list = []
    singleTest_list = []
    ...
    for lowdata in lowDataList:
        ...
        if lowdataIndex % 4 != 0:
            ...
            tempData.append(ASCNN_Frames_List1[ASCNN_Frames_index])
            tempData.append(lowdata)
            tempData.append(ASCNN_Frames_List2[ASCNN_Frames_index])
...
def getBeforeNNBlockDict(img, w, h):
    ...
    for yBlock in range(yBlockNum):
        for xBlock in range(xBlockNum):
            ...
            tempImg[i] = np.reshape(tempImg[i], (1, np.shape(tempImg[i])[0], np.shape(tempImg[i])[1], 1))
            i += 1
            # print(np.shape(tempImg[i]))
            # tempImg[i] =  tempImg[i] * 255
